...
MathFlux is free to download and play. Each competition match costs one
token. Every player gets a free token every day just for opening the app,
plus a few welcome tokens to start — or top up anytime with optional token
packs. A one-time Premium unlock adds the Middle School and College content
tiers and includes a one-time token bonus; competitions still use tokens.
No ads. No tracking. No accounts to create. We use anonymous authentication
only.
...
|------------|-------|--------|
| `Samuel.Praxis_Math.premium` | $4.99 | Sets `isPremium=true` permanently — unlocks the Middle School + College content tiers and grants a one-time token bonus (competitions still use tokens) |
